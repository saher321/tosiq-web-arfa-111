const BASE_URL_V1 = "http://localhost:5000/api/v1"

// auth
export const ALL_USER_API = BASE_URL_V1 + "/users"
export const REG_USER_API = BASE_URL_V1 + "/auth/register"
export const LOG_USER_API = BASE_URL_V1 + "/auth/login"
export const FPS_USER_API = BASE_URL_V1 + "/auth/forgot-password"
export const RST_USER_API = BASE_URL_V1 + "/auth/reset-password"

// cusomters
export const ALL_CUST_API = BASE_URL_V1 + '/customers'
export const ADD_CUST_API = BASE_URL_V1 + '/customers/add'
export const DEL_CUST_API = BASE_URL_V1 + '/customers'
export const EDT_CUST_API = BASE_URL_V1 + '/customers'

// projects

// tasks